...
# stated completion date without filing any explanation
...
AGENT_TOOLS = [
    ...
    # or silent (no amendment, no explanation)
]
...
async def timeline_node(state: GraphState) -> dict:
    """
    the Timeline Agent node - detects silent delays in clinical trials
    ...
    try:
        ...
        system_prompt = f"""You are the Timeline Analyst Agent for this
...
5. If no amendment and significantly delayed → generate signal
   - EXCEPTION: COVID-19 exception - delays between March 2020 and December 2022 
                often had legitimate COVID-related causes - ignore these
...
"""
        ...
        await _episodic.save_episode(
            ...
            episodic_content = f"Task: {task}. Found {len(signals_found)} timeline delay signals.",
