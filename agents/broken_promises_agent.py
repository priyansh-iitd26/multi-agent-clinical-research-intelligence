...
# plan to measure as the primary outcome - example: "Reduction in HbA1c at 26 weeks"
...
async def broken_promises_node(state: GraphState) -> dict:
    """
    Broken Promises Agent node - detects outcome switching in clinical trials
    it runs in parallel with the other 5 specialist agents
    """
    ...
    try:
        ...
        system_prompt = f"""You are the Broken Promises Agent for this clinical trials
...
"""
        ...
        await _episodic.save_episode(
            ...
            episodic_content = f"Task: {task}. Found {len(signals_found)} broken promise signals.",
