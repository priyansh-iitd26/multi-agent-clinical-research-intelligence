# finds cross-study patterns that no single human analyst reading one study
...
async def pattern_finder_node(state: GraphState) -> dict:
    """
    the Pattern Finder Agent node - detects cross-study research patterns
    ...
    try:
        ...
        system_prompt = f"""You are the Pattern Finder Agent for this
...
- Below 0.65: Only 2 studies or weak connection
...
"""
        ...
        await _episodic.save_episode(
            ...
            episodic_content = f"Task: {task}. Found {len(signals_found)} cross-study pattern signals.",
