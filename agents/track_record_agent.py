# builds and evaluates the entire sponsor credibility profiles based
# on their entire history of behaviour across all studies in our database
...
# this agent looks at the FULL PICTURE across all a sponsor's studies -
...
async def track_record_node(state: GraphState) -> dict:
    """
    the Track Record Agent node - evaluates sponsor credibility
    ...
    try:
        ...
        system_prompt = f"""You are the Track Record Agent for this
clinical trials research integrity system.
...
"""
        ...
        for iteration in range(max_iterations):
            ...
            if not response.tool_calls:
                logger.info(f"{AGENT_NAME} | Analysis completed! | iteration = {iteration+1}")
        ...
        await _episodic.save_episode(
            ...
            episodic_content = f"Task: {task}. Found {len(signals_found)} low credibility signals.",
            outcome = "signal_generated" if signals_found else "no_signal"
        )

        logger.info(f"{AGENT_NAME} | Completed! | signals_found = {len(signals_found)}")
